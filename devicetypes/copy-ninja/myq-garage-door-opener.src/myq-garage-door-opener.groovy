...
metadata {
    ...
    multiAttributeTile(name:"door", type:"generic", width:6, height:4) {
      tileAttribute("device.door", key: "PRIMARY_CONTROL") {
        attributeState("unknown", label:'${name}', action:"door control.close", icon:"st.doors.garage.garage-open",    backgroundColor:"#ffa81e")
        ...
        attributeState("opening", label:'${name}', action:"door control.close", icon:"st.doors.garage.garage-opening", backgroundColor:"#ffe71e")
        attributeState("closing", label:'${name}', action:"door control.open", icon:"st.doors.garage.garage-closing", backgroundColor:"#ffe71e")
        attributeState("stopped", label:'Stopped', action:"door control.close", icon:"st.doors.garage.garage-opening", backgroundColor:"#1ee3ff")
      }
      tileAttribute("device.lastActivity", key: "SECONDARY_CONTROL") {
        attributeState "default", label: '${device.door.name} for ${currentValue}'
      }
    }
